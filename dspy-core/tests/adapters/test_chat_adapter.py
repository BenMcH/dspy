...
def test_chat_adapter_quotes_literals_as_expected(
    ...
    """Verify that Literal field types are correctly formatted in the system message."""
    ...
    content = messages[0]["content"]  # system message

    assert expected_input_str in content
    assert expected_output_str in content


@pytest.mark.skip(reason="Requires dspy.LM / adapter.__call__ which is not available in dspy-core")
def test_chat_adapter_sync_call():
    pass


@pytest.mark.skip(reason="Requires dspy.LM / adapter.acall which is not available in dspy-core")
@pytest.mark.asyncio
...
def test_chat_adapter_signature_information():
    ...
    adapter = dspy.ChatAdapter()
    messages = adapter.format(TestSignature, [], {"input1": "Test", "input2": 11})
...
@pytest.mark.skip(reason="Requires dspy.LM / _call_preprocess which is not available in dspy-core")
def test_chat_adapter_native_reasoning():
    pass


def test_chat_adapter_parses_float_with_underscores():
    """ChatAdapter can parse float numbers with underscores via parse()."""
    ...
    adapter = dspy.ChatAdapter()
    completion = "[[ ## score ## ]]\n{'score': 123_456.789}\n[[ ## completed ## ]]"
    result = adapter.parse(MySignature, completion)
    assert result["score"].score == 123456.789
