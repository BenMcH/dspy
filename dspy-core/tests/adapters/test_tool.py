...
from dspy_core.adapters.types.tool import Tool, ToolCalls, convert_input_schema_to_tool_args
...
def test_async_tool_call_in_sync_mode():
    tool = Tool(async_dummy_function)
    with pytest.raises(ValueError, match=r".*acall.*allow_async_sync_conversion.*"):
        result = tool(x=1, y="hello", allow_async_sync_conversion=False)

    result = tool(x=1, y="hello", allow_async_sync_conversion=True)
