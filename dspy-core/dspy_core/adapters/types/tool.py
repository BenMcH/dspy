...
from dspy_core.adapters.types.base_type import Type
from dspy_core.utils.callback import with_callbacks
...
class Tool(Type):
    ...
    def to_function_call_schema(self):
        """Return a generic OpenAI-compatible function call schema dict."""
        return {
            "type": "function",
            "function": {
                "name": self.name,
    ...
    def format_as_litellm_function_call(self):
        return self.to_function_call_schema()
    ...
    @with_callbacks
    def __call__(self, allow_async_sync_conversion: bool = False, **kwargs):
        parsed_kwargs = self._validate_and_parse_args(**kwargs)
        result = self.func(**parsed_kwargs)
        if asyncio.iscoroutine(result):
            if allow_async_sync_conversion:
                return self._run_async_in_sync(result)
            else:
                raise ValueError(
                    "You are calling `__call__` on an async tool, please use `acall` instead or pass "
                    "`allow_async_sync_conversion=True` to enable async-to-sync conversion."
                )
        return result
    ...
    def from_mcp_tool(cls, session: "mcp.ClientSession", tool: "mcp.types.Tool") -> "Tool":
        ...
        Returns:
            A Tool object.
        """
        from dspy_core.utils.mcp import convert_mcp_tool
    ...
    def from_langchain(cls, tool: "BaseTool") -> "Tool":
        ...
        ```
        """
        from dspy_core.utils.langchain_tool import convert_langchain_tool
