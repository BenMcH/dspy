...
if TYPE_CHECKING:
    ...
    from dspy_core.adapters.types.tool import Tool
...
class BaseCallback:
    ...
    For example, if you want to print a message before and after an adapter's format() is called,
    implement the `on_adapter_format_start` and `on_adapter_format_end` handlers.
    Pass the callback to the component constructor.


    Example: Set a local callback by passing it to the adapter constructor.

    ```
    from dspy_core.adapters import JSONAdapter
    from dspy_core.utils.callback import BaseCallback

    class LoggingCallback(BaseCallback):

        def on_adapter_format_start(self, call_id, instance, inputs):
            print(f"Adapter format called with inputs: {inputs}")

        def on_adapter_format_end(self, call_id, outputs, exception):
            print(f"Adapter format finished")

    adapter = JSONAdapter(callbacks=[LoggingCallback()])
    ```
    """
...
def with_callbacks(fn):
    ...
    def _get_active_callbacks(instance):
        ...
        return getattr(instance, "callbacks", [])
